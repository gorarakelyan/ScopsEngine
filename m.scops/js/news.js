...
*/
	...
	function watchMedia( postID , parent, postBool , mediaType , turn ) {
		...
		$.post( _root_ + '/php_requests/get_content.php',{ log_link: getCookie('link') , mobile_application: true , content: 'media_view', post_id: postID , media: mediaType },function(dataRes){
			...
			if( !dataJSON.media.empty ) {
				...
				} else  startVideo( _root_ + '/' + scriptData );
	...
	function changePhotosGallery( elem ,scriptData, num ) {
		elem.find('.img').html( '<img src="' + _root_ + '/' + scriptData[num] + '">' );
	...
	function openNews( id ){
		...
		$.post( _root_ + '/php_requests/get_content.php' , { log_link: getCookie('link') , mobile_application: true , content: 'news_view', post_id: id } , function(dataRes){
			...
			if( dataJSON.news.blocked_me || !dataJSON.news.isset ) {
				...
				var elem = $( Mustache.to_html( tpl , $.extend( {}, dataJSON.news , { langs: langs } , { main_set: dataJSON.main_set } ) ) );
				...
				if( dataJSON.news.shared != '' ) {
					if( !dataJSON.news.share_array.blocked_me ) {
						...
						var share = $( Mustache.to_html( tpl , $.extend( {},  dataJSON.news.share_array , { langs: langs } , { main_set: dataJSON.main_set } ) ) );
				...
				fillDataTPL( _root_ + '/php_requests/comments.php',{type: 'fill', length: loadContentLengths[6], 'from': 'NULL', article: id },'fillCOMM(\'.comments-show-area .comments-box\' , ' + id +')');
		...
		if( $('.comments-box').innerHeight() - $('#page-content').scrollTop() - $(window).height() < 300 && !commentsEnd ) {
			if( commentsLoaded ) {
				...
				if( lastID ) {
					...
					fillDataTPL( _root_ + '/php_requests/comments.php',{type: 'fill', length: loadContentLengths[6], 'from': lastID, article: id },'fillCOMM(\'#news-'+ id +' .comments-show-area .comments-box\' )');
				}
			}
		}
	}
	...
	function sendComm( event, id ) {
		if( event.keyCode == 13 && !event.shiftKey ) {	
			...
			$.post( _root_ + '/php_requests/comments.php',{ log_link: getCookie('link') , mobile_application: true , type: 'send', article: id, text: val  });
	...
	function startListenComms( id ) {
		...
		commAjax = $.post( _root_ + '/php_requests/comments.php',{ log_link: getCookie('link') , mobile_application: true , type: 'listen', article: id, comm: lastComm },function(data){
			...
			bg.onload = function(){
				...
				<div id="img-thumb-form">\
					...
					<div id="resize-choose">\
						<img id="bg-img" src="' + _root_ + '/'+ imgLink +'">\
						<div id="drag-area">\
							<div id="drag" style="width:'+ dragWidth +'px">\
								<div class="thumb-box">\
									<img class="thumb" src="' + _root_ + '/'+ imgLink +'">\
								</div>\
							</div>\
						</div>\
					...
					<div id="resize-result">\
						<div id="resize-img" style="background:url(\'' + _root_ + '/'+ imgLink +'\');background-repeat: no-repeat;"></div>\
	...
	function saveThumb( id , type , author ) {
		...
		if( author ) {
			...
			$.post( _root_ + '/php_requests/upload_post.php',{ log_link: getCookie('link') , mobile_application: true , img: JSON.stringify( array ), id: id, type: 'thumb', photo: photo, club: clubID },function(data){
				window.location.reload();
			});
			
		} else {
			$.post( _root_ + '/php_requests/upload_post.php',{ log_link: getCookie('link') , mobile_application: true , img: JSON.stringify( array ), id: id, type: 'thumb', photo: photo },function(data){
				window.location.reload();
			});
		}
	}
	...
	function likePost( type , id ) {
		if( firstClick ) {
			...
			setTimeout(function(){
				...
				if( !secondClick ) request( type , id );
				else {
					if( $('#pages #pop-page #page-content table').length ) {
						closeNews( true );
						$('#pages #win-page .shadow-object').bind('click' , function(){
						...
						openWinPage( 1 );
					} else openWinPage( 1 , true );
					loader( '#pages #win-page #win-content' );
					fillDataTPL( _root_ + '/php_requests/get_content.php' , { content: 'likes' , like_type: type == 3 ? 'likes' : 'unlikes' , post_id: id } , 'fillJSON(\'#pages #win-page #win-content\')' );
				}
