...
*/
  ...
  function notification( type ) {
    ...
    if( notifIsOpened ) {
      ...
      fillDataTPL( _root_ + '/php_requests/get_content.php',{ content: 'notif_' + type , length: loadContentLengths[9] , last_id: 'NULL' },'fillJSON(\'#pages #panel-content\' , false , false , false , [ 0 , true , false ] , false , true )');
      
      var wordElem = $('#pages #panel-page .panel-header .cell.name');
      switch( type ) {
        case 'msg': wordElem.text( langs.messages ); break;
        case 'notif': wordElem.text( langs.notifications ); break;
        case 'visits': wordElem.text( langs.visitors ); break;
        case 'requests': wordElem.text( langs.requests ); break;
      }
  ...
  function openHashtag( name ) {
    $('#panels #main-panel .item.search .find').val( name );
    ajaxQuery('search' , 4 );
  }
  
  function openMenu() {
    if( !menuOpen ) {
      $('#panels #main-panel').css('left' , '0px' );
      setTimeout(function(){ menuOpen = true; },100);
    }
  }
  
  function closeMenu() {
    $('#panels #main-panel').css('left' , '-450px' );
    menuOpen = false;
  }
  
  function openPlayer() {
    if( !musicTab ) {
      $('#pages #audioplayer .playlist').outerHeight( $(window).height() - $('#pages #audioplayer .header').outerHeight(true) );
      $('#pages #audioplayer').css('left' , '0%' );
      $('body').css('overflow','hidden');
  ...
  function closePlayer() {
    $('#pages #audioplayer').css('left','-150%');
    musicTab = false;
