{{#user}}
	<div id="user-profile" class="material-object">
		...
		<div id="user-profile-options">
				...
				<div id="user-profile-img" data-url="{{img}}" style="background:url('../{{img}}');background-repeat:no-repeat;background-position: 50% 50%;background-size:cover;">
					{{^blocked_me}}
						<div class="user-img-options {{#me}}nemp{{/me}} {{#img_exist}}nemp{{/img_exist}}">
							{{#me}}<div class="upload" onclick="choosePhotoFrom(0,0)"></div>{{/me}}
			...
			<div class="user-option-object text">{{#online}}<div class="online"></div>{{/online}}{{name}}{{#verified}}<span class="verified"></span>{{/verified}}</div>
		</div>
	...
	{{^blocked_me}}
		...
		<div id="user-profile-controls">
			<div id="user-menu">
				<div class="menu-block" onclick="showForm()">
				...
				<div class="menu-block" id="clubs" onmouseover="menuTransition( 'clubs' )" onclick="changeContent( 'clubs', 'user' )" onmouseout="endMenuTransition()">
	...
	{{#user_info}}
	<div id="user-profile-info" class="hide opacity">
