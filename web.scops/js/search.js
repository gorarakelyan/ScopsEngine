...
*/
	...
	var panelSearch = $('#main-panel #input-panel-search');
	...
	searchForm.ajaxForm({
		success: function( data ){
			...
			switch( searchTab ) {
				case 4:
					generateFeed('../php_requests/search.php',{ name: searchForm.find('#name').val(), type: 'tag' , length: loadContentLengths[0] , last_id: 'NULL' } , $('#search-result') , false , true );
				break;
				case 3:
	...
	function changeSearchTab( num ) {
		...
		continueWall = false;
		searchTab = num;
		$('#search-columns .tab').attr('class','tab simple-btn-object');
		$('#search-columns .tab').eq(num).attr('class','tab input-btn-object');
		$('#search-area #search-form').html( getSearchTPL()[num] );
